...
def mapa_mensal():
    ...
    if opcao == 'Indices':
        with st.form(key='form_indice'):
            #ticker = st.delectionbox('Indice', ['Bovespa', 'Financial', 'Basic Materials'])
            ticker = st.selectbox('Indice', ['^BVSP', '^GSPC', '^SP500-15'])
...
#                                                       interval='Monthly')['Close'].pct_change()
            retornos = yf.download(ticker, start=start_date, end=end_date)['Close'].pct_change()
